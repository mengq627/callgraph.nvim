...
- GitHub Actions (`.github/workflows/ci.yml`): the `test` job runs the full suite + coverage summary (hard); the `coverage` job (`continue-on-error`) runs on push and PR and shows the result without blocking. `luacov.report.out` is uploaded as a build artifact.
