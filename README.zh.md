...
| 键 | 动作 |
|---|---|
| `h` `j` `k` `l` / 方向键 | 移动选中框（同行/同列对齐优先，其次空间最近） |
...
| `+` / `=` / `-` | 全局深度 +1 / -1 |
| `<` / `>` | 放大 / 缩小视图窗口（right 布局调宽度，bottom 布局调高度） |
| `?` | 按键速查 |
...
- **动态窗口大小**：`<` / `>` 手动放大/缩小视图窗口（right 布局调宽度、bottom 布局调高度），尺寸记入配置，后续渲染与重新打开保持。
